...
(function () {
  ...
  /* ---------------- RECENT ACHIEVEMENTS — 3D ROTATING CAROUSEL ---------------- */
  const rcRing = $("#rcRing");
  if (rcRing) {
    const rcCards = $$(".rc-card", rcRing);
    const rcStep = 360 / rcCards.length;
    let rcRot = 0, rcVel = 0, rcDrag = false, rcLastX = 0, rcIdle = 0, rcMoved = 0;
    function rcLayout() {
      const Ww = rcRing.clientWidth || 800;
      const cw = rcCards[0].clientWidth || 330;
      const R = cw < 280 ? Math.max(200, Math.round(cw * 1.02)) : Math.min(430, Math.max(300, Ww * 0.42));
      rcCards.forEach((c, i) => {
        c.style.transform = "rotateY(" + (i * rcStep) + "deg) translateZ(" + R + "px)";
      });
    }
    rcLayout();
    window.addEventListener("resize", rcLayout);
    rcRing.addEventListener("pointerdown", (e) => {
      rcDrag = true; rcMoved = 0; rcVel = 0; rcIdle = 0; rcLastX = e.clientX;
    ...
    rcRing.addEventListener("pointermove", (e) => {
      ...
      rcRot += dx * 0.35; rcVel = dx * 0.35; rcIdle = 0;
    });
    ...
    (function rcFrame() {
      if (!rcDrag) {
        rcVel *= 0.95;
        rcRot += rcVel + (reduced ? 0 : 0.06);
      }
      rcRing.style.transform = "rotateY(" + rcRot + "deg)";
      rcCards.forEach((c, i) => {
        const a = ((i * rcStep + rcRot) % 360 + 360) % 360;
        c.classList.toggle("front", a < rcStep * 0.5 || a > 360 - rcStep * 0.5);
      });
      requestAnimationFrame(rcFrame);
    })();
  }
  ...
  const boot = $("#boot");
  if (boot) {
    const bootPct = $(".boot-pct", boot);
    const bootFill = $(".boot-fill", boot);
    const bootStat = $(".boot-stat", boot);
    const BOOT_MSGS = ["CALIBRATING STARFIELD", "CHARGING NEBULA", "POLISHING PIXELS", "OPENING THE ROAD EAST", "FINALIZING UNIVERSE"];
    let bootDone = false;
    function hideBoot() {
      if (bootDone) return;
      bootDone = true;
    ...
    if (reduced) {
      ...
      const msgTimer = setInterval(() => {
        mi = (mi + 1) % BOOT_MSGS.length;
        bootStat.textContent = BOOT_MSGS[mi];
      }, 480);
      const t0 = performance.now(), DUR = 2000;
      (function tick(t) {
        const p = clamp(((t || 0) - t0) / DUR, 0, 1);
        const eased = p * p * (3 - 2 * p) * 100;
        bootPct.textContent = Math.floor(eased) + "%";
        bootFill.style.width = eased + "%";
        if (p < 1) requestAnimationFrame(tick);
        else setTimeout(() => { clearInterval(msgTimer); hideBoot(); }, 300);
      })(t0);
  ...
  if (p3d) {
    ...
    const pSpin = reduced ? 0 : 0.15; /* permanent auto-rotation (~10s per face) */
    (function pLoop() {
