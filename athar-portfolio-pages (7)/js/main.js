...
(function () {
  ...
  /* ---------------- CERTIFICATE SHOWCASE — VERTICAL ROTATING STACK ---------------- */
  const rcRing = $("#rcRing");
  if (rcRing) {
    const rcCards = $$(".rc-card", rcRing);
    const N = rcCards.length;
    const rcStage = rcRing.parentElement;
    let rcFocus = 0, rcDrag = false, rcLastY = 0, rcMoved = 0, rcAcc = 0, rcHover = false;
    function rcOffset(i) {
      let o = (i - rcFocus) % N;
      if (o > N / 2) o -= N;
      if (o < -N / 2) o += N;
    ...
    function rcD() {
      const cH = rcCards[0].clientHeight || 300;
      const S = (rcStage.clientHeight || 360) / 2;
      return Math.max(0.9, (S - 26 + (cH / 2) * 0.86) / cH);
    }
    function rcApply(animate) {
      ...
      rcCards.forEach((c, i) => {
        ...
        let s = 1, op = 1, bl = 0;
        if (abs === 1) { s = 0.86; op = 0.5; bl = 2.5; }
        else if (abs > 1) { s = 0.8; op = 0; bl = 3; }
        if (!animate) c.style.transition = "none";
        c.style.transform = "translate(-50%," + (-50 + o * d * 100) + "%) scale(" + s + ")";
        c.style.opacity = op;
        c.style.filter = bl ? "blur(" + bl + "px) brightness(.7)" : "none";
    ...
    rcRing.addEventListener("pointerdown", (e) => {
      rcDrag = true; rcMoved = 0; rcAcc = 0; rcLastY = e.clientY;
      try { rcRing.setPointerCapture(e.pointerId); } catch (err) {}
    });
    rcRing.addEventListener("pointermove", (e) => {
      if (!rcDrag) return;
      const dy = e.clientY - rcLastY; rcLastY = e.clientY; rcMoved += Math.abs(dy);
      if (Math.abs(rcAcc) >= 55) { rcNext(rcAcc < 0 ? 1 : -1); rcAcc = 0; }
      else rcAcc += dy;
    });
