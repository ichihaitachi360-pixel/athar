...
(function () {
  ...
  if (rcRing) {
    ...
    rcCards.forEach((c) => c.addEventListener("click", (e) => {
      if (rcMoved > 8) return;
      if (e.target.closest(".rc-open")) return; /* OPEN CERTIFICATE acts normally */
      e.preventDefault();
      const vault = document.getElementById("certifications");
      if (vault) vault.scrollIntoView({ behavior: reduced ? "auto" : "smooth", block: "start" });
    }));
    /* OPEN CERTIFICATE / OPEN PDF buttons: never bubble to the card's vault-scroll */
    $$(".rc-card .rc-open").forEach((a) => a.addEventListener("click", (e) => e.stopPropagation()));
    setInterval(() => {
  ...
  if (p3d) {
    ...
    const pSpin = reduced ? 0.09 : 0.22; /* permanent auto-rotation — both photos cycle every ~15s so the cyber face is always seen */
    (function pLoop() {
