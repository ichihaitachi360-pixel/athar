...
(function () {
  ...
  /* ---------------- BOOT SEQUENCE — “Athar” signature (programming style) ---------------- */
  (function boot() {
    const el = $("#boot");
    if (!el) return;
    let ended = false;
    ...
    if (reduced) { setTimeout(done, 400); return; }
    const t1 = $("#bootType1"), t2 = $("#bootType2");
    const s1 = t1.getAttribute("data-text") || "whoami";
    let i1 = 0, i2 = 0;
    (function type1() {
      if (ended) return;
      if (i1 <= s1.length) { t1.textContent = s1.slice(0, i1); i1++; setTimeout(type1, 70); }
      else setTimeout(type2, 240);
    })();
    function type2() {
      if (ended) return;
      if (i2 <= 5) { t2.textContent = "Athar".slice(0, i2); i2++; setTimeout(type2, 135); }
